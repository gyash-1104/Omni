...
def _resolve_admin_auth(
    ...
    """Shared auth: X-Admin-Key, Bearer header, or ?token= for SSE."""
    if api_key and api_key == settings.admin_api_key:
...
async def require_admin(
    ...
    return _resolve_admin_auth(request, api_key)
...
async def require_admin_sse(
    ...
    """Admin check for SSE — supports ?token= query (EventSource cannot send headers)."""
    return _resolve_admin_auth(request, api_key, query_token=token)
